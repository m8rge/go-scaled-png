...
func (d *decoder) readImagePass(r io.Reader, pass int, allocateOnly bool) (image.Image, error) {
	...
	if d.targetHeight > 0 && d.targetHeight < d.height && d.interlace == itNone {
		...
		case *image.NRGBA64:
			VerticalNRGBA64InPlaceQ15(im, d.targetHeight, d.filter)
		case *image.RGBA64:
			VerticalRGBA64InPlaceQ15(im, d.targetHeight, d.filter)
		case *image.Gray16:
			VerticalGray16InPlaceQ15(im, d.targetHeight, d.filter)
		}
	}
...
// If targetWidth/targetHeight are > 0, the decoder may scale while decoding.
// MVP: only 8-bit decode paths (cbTC8 / cbTCA8 / cbG8) are supported here.
// Pass filter for resampling (e.g. ImagingFilter(Lanczos.Support, Lanczos.Kernel)).
