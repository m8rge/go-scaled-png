...
func TestIncompleteIDATOnRowBoundary(t *testing.T) {
	...
	_, err := Decode(strings.NewReader(pngHeader+ihdr+idat+iend), nil)
	if err == nil {
...
func TestTrailingIDATChunks(t *testing.T) {
	...
	_, err := Decode(strings.NewReader(pngHeader+ihdr+idatWhite+idatZero+iend), nil)
	if err != nil {
	...
	img, err := Decode(strings.NewReader(pngHeader+ihdr+idatWhite+idatBlack+iend), nil)
	if err != nil {
...
func TestMultipletRNSChunks(t *testing.T) {
	...
	*/
	...
	for i := 0; i < 4; i++ {
		...
		m, err := Decode(bytes.NewReader(b), nil)
		switch i {
		case 0:
			if err != nil {
...
// 		if _, err := Decode(bytes.NewReader(tc.src), nil); err == nil {
...
// 	// }
//
// 	img, err := Decode(bytes.NewReader(src), nil)
// 	if err != nil {
// 		t.Fatalf("Decode: %v", err)
